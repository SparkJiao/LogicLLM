...
logger = get_child_logger("Wiki.Entity.Path.V9.0")
...
def replace_ent_neg_double(candidate, h_ent_id, h_ent_str, t_ent_id, t_ent_str, rep_pairs: Dict[int, str] = None,
    ...
    for _perm in _source_ls:
        ...
        for tgt in tgt_ls:
            ...
            # FIXME (V2): We modify here for test
            if not out_of_domain:
                _cur_rep_pairs_copy = copy.deepcopy(rep_pairs) if rep_pairs is not None else {}
            else:
                _cur_rep_pairs_copy = {}
                if rep_pairs is not None:
                    _all_ent_strs = list(rep_pairs.values())
                    for ent_id in filtered_entities:
                        _cur_rep_pairs_copy[ent_id] = random.choice(_all_ent_strs)
...
def add_noise_sentence(num: int, item: Dict, orig_sentences: List[str], rep_pairs: Dict[int, str] = None, shuffle: bool = False) -> str:
    ...
    for noise_tgt, noise_s in zip(noise_tgt_sent, noise_src_sent):
        _res = replace_neg(noise_tgt, noise_s, rep_pairs=rep_pairs, out_of_domain=True)
        if len(_res) > 0:
...
def _process_single_item(item, max_neg_num: int, aug_num: int, min_rep_num: int, shuffle_context: bool,
    ...
    _all_ent_ids = list(set(item["entity"].keys()))
    ...
    for _ in range(aug_num):  # Repeat for augmentation
        ...
        for pos_idx, pos_candi in enumerate(item["pos"]):
            ...
            target_ent_str = sample_entity(_entity_pool, item["id"], len(_all_ent_ids))
            sampled_rep_pairs = {_ent_ids: _rep_str for _ent_ids, _rep_str in zip(_all_ent_ids, target_ent_str)}
            ...
            while len(neg_res) < MAX_NEG_SAMPLE_NUM:
                ...
                # FIXME (V2):
                _rep_res = replace_neg(sampled_neg_candidates[0], neg, rep_pairs=_cur_aug_rep_pairs, out_of_domain=True)
                if _rep_res:
                    neg_res.extend(_rep_res)
            ...
            if len(neg_ctx_sent) < MAX_NEG_SAMPLE_NUM:
                for neg in sampled_neg_candidates + _all_neg_candidates[_sampled_neg_item_key]:
                    ...
                    _rep_res, _ = context_replace_neg(tgt_ctx_sent, neg, rep_pairs=_cur_aug_rep_pairs, out_of_domain=True)
                    _rep_res = [(tgt_ctx_sent_id, _tmp_res) for _tmp_res in _rep_res]
...
def convert_examples_into_features(file_path: str, tokenizer: PreTrainedTokenizer,
    ...
    file_suffix = f"{tokenizer_name}_{shuffle_context}_{max_neg_num}_{aug_num}_" \
                  ...
                  f"{'' if not remove_context else 'no-ctx-ex_'}{'' if not remove_deduct else 'no-duc-ex_'}path_v9_ent_rep_test_v2"
    ...
    if os.path.exists(cached_file_path):
        ...
        #     if "h" in example and "t" in example:
        #         _new_examples.append(example)
        #
        # dataset = WikiPathDatasetV5(_new_examples, raw_texts)
    ...
    # return WikiPathDatasetV5(_new_examples, raw_texts)
...
def _quick_loading_keep_aug_only(file_path: str, **kwargs):
    logger.info(f"Quickly loading cached file from {file_path}")
    all_examples, raw_texts = torch.load(file_path)
