...
from data.collators.wiki import WikiPathDatasetGenerate
...
Version 8.2-generate:
    1.  Remove concatenation of different sentences for generation sampling.
        Currently, we don't add noise sentence because we need to select the correct sentences for generation.
"""

logger = get_child_logger("Wiki.Entity.Path.V8.2.Gen")
...
def _process_single_item(item, max_neg_num: int, aug_num: int, min_rep_num: int, shuffle_context: bool,
    ...
    context_sent = [" ".join(s["sent"]) for s_id, s in selected_sentences.items()]
...
def convert_examples_into_features(file_path: str, tokenizer: PreTrainedTokenizer,
    ...
    file_suffix = f"{tokenizer_name}_{shuffle_context}_{max_neg_num}_{aug_num}_" \
                  ...
                  f"{'' if not remove_context else 'no-ctx-ex_'}{'' if not remove_deduct else 'no-duc-ex_'}path_v8_2_generate"

    cached_file_path = f"{file_path}_{file_suffix}"
    if os.path.exists(cached_file_path):
        logger.info(f"Loading cached file from {cached_file_path}")
        all_examples = torch.load(cached_file_path)
        dataset = WikiPathDatasetGenerate(all_examples)
        return dataset

    examples, context_examples = read_examples(file_path, shuffle_context=shuffle_context, max_neg_num=max_neg_num,
    ...
    torch.save(all_examples, cached_file_path)

    return WikiPathDatasetGenerate(all_examples)
