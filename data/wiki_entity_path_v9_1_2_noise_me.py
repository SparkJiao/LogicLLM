...
Version v9.1.2_noise:
    1.  Add noisy sentences.
    
    TODO: Maybe we could make further improvements by introducing the negative samples from the same paragraph.
        But this may not be useful. We need to first verify that enough negative candidates coming from different paragraphs.
"""
...
def sample_context_sentence(num: int, cur_item):
    para_id = get_para_id(cur_item)
    if len(_all_neg_candidates_para[para_id]) <= 1:
        return []

    para_keys = set(_all_neg_candidates_para[para_id].keys())
    para_keys.remove(cur_item["id"])
    para_keys = list(para_keys)

    res = []
    for sampled_item_key in para_keys:
        sample_num = min(num, len(_all_neg_candidates_para[para_id][sampled_item_key]))
        res.extend(random.sample(_all_neg_candidates_para[para_id][sampled_item_key], sample_num))
        if len(res) >= num:
            break
        num -= sample_num

    assert len(res) <= num, (len(res), num)

    return res


def insert_sentences(orig_sentences: List[Dict[str, Any]],
                     orig_sentence_ids: List[str],
                     new_sentences: List[Dict[str, Any]],
                     shuffle: bool = True) -> Tuple[List[Dict[str, Any]], List[str]]:
    assert len(orig_sentences) == len(orig_sentence_ids), (len(orig_sentences), len(orig_sentence_ids))
    ...
    for x in index:
        if x == 0:
            ...
            res_s_ids.append(orig_sentence_ids[orig_idx])
            orig_idx += 1
        else:
    ...
    assert len(res) == len(res_s_ids)
    return res, res_s_ids


def add_noise_sentence(num: int, item: Dict, orig_sentences: List[Dict[str, Any]], orig_sentence_ids: List[str],
                       rep_pairs: Dict[int, str] = None,
                       shuffle: bool = False) -> Tuple[List[Dict[str, Any]], List[str]]:
    if num == 0:
        return orig_sentences, orig_sentence_ids

    noise_src_sent = sample_context_sentence(num, item)
    if len(noise_src_sent) == 0:
        return orig_sentences, orig_sentence_ids
    ...
    for noise_tgt, noise_s in zip(noise_tgt_sent, noise_src_sent):
        ...
        if len(_res) > 0:
            ...
            _res["h"] = "-1"
            _res["t"] = "-1"
            noise_sent.append(_res)

    noise_ctx, noise_ctx_s_ids = insert_sentences(orig_sentences, orig_sentence_ids, noise_sent, shuffle=shuffle)

    return noise_ctx, noise_ctx_s_ids
...
def extract_sent_entity(sent, rep_pairs: Dict[int, str] = None):
    ent_id2mention = collections.defaultdict(set)
    for ent_id, ent_mentions in sent["ent"].items():
        for mention in ent_mentions:
            if rep_pairs is not None and ent_id in rep_pairs:
                ent_id2mention[ent_id].add(rep_pairs[ent_id])
            else:
                ent_id2mention[ent_id].add(pos2str(mention["pos"][0], mention["pos"][1], sent["sent"]))
    return ent_id2mention


def update_ent_id2mention(old: Dict[str, set], new: Dict[str, set]):
    for new_ent_id, new_ent_mentions in new.items():
        if new_ent_id not in old:
            old[new_ent_id] = copy.deepcopy(new_ent_mentions)
        else:
            old[new_ent_id].update(new_ent_mentions)
    return old


def _initializer(entity_pool: Dict, negative_pool: Dict, all_neg_candidates: Dict, all_path_sentences: Dict,
                 all_neg_candidates_para: Dict,
                 geometric_dist: torch.distributions.Distribution,
                 max_neg_samples_num: int):
    global _entity_pool
    global _negative_pool
    global _all_neg_candidates
    global _all_neg_candidates_para
    ...
    _all_neg_candidates_para = all_neg_candidates_para
...
def _process_single_item(item, max_neg_num: int, aug_num: int, min_rep_num: int, shuffle_context: bool,
                         ...
                         remove_deduct: bool = False, remove_context: bool = False, para_sample_neg: bool = False):
    examples = []
    context_examples = []
    ...
    for pos_idx, pos_candi in enumerate(item["pos"]):
        ...
        ent_id2mention_ls = {}
        for s_id in rel_s_ids_order:
            ent_id2mention_ls = update_ent_id2mention(ent_id2mention_ls,
                                                      extract_sent_entity(selected_sentences[s_id], rep_pairs=None))
        ent_id2mention_ls = update_ent_id2mention(ent_id2mention_ls,
                                                  extract_sent_entity(pos_candi, rep_pairs=None))
        me_target_id = random.choice([pos_candi["h"], pos_candi["t"]])
        ...
        noised_sentences, noised_s_ids = add_noise_sentence(noise_sent_num, item, orig_sentences,
                                                            sent_order2s_id, rep_pairs=None, shuffle=True)

        _r = random.random()
        if not remove_deduct and _r < deduct_ratio:
            examples.append({
                "context": noised_sentences,
                "negative": neg_res,
                ...
                "path_s_ids": noised_s_ids,
                "path_s_ids_order": rel_s_ids_order,
                "pos_id": pos_idx,
                "ent_mentions": path_ent_str_ls,
                "ent_id2mention": ent_id2mention_ls,
                "me_target_id": me_target_id,
                "me_target_sent": "pos",
            })
        ...
        if not remove_context and _r < context_ratio:
            ...
            negative_noise_context = [
                add_noise_sentence(noise_sent_num, item, neg_ctx, sent_order2s_id,
                                   rep_pairs=None, shuffle=True)[0] for neg_ctx in negative_context
            ]

            noised_sentences, noised_s_ids = add_noise_sentence(noise_sent_num, item, orig_sentences, sent_order2s_id,
                                                                rep_pairs=None, shuffle=True)

            tmp = list(enumerate(noised_s_ids))
            me_target_sent = random.choice(tmp)
            while me_target_sent[1] == "-1":
                me_target_sent = random.choice(tmp)
            me_target_sent = me_target_sent[0]
            me_target_id = random.choice([noised_sentences[me_target_sent]["h"], noised_sentences[me_target_sent]["t"]])
    ...
    for _ in range(aug_num):  # Repeat for augmentation
        ...
        for pos_idx, pos_candi in enumerate(item["pos"]):
            ...
            for s_id in rel_s_ids_order:
                ent_id2mention_ls = update_ent_id2mention(ent_id2mention_ls,
                                                          extract_sent_entity(selected_sentences[s_id], _cur_aug_rep_pairs))
            ent_id2mention_ls = update_ent_id2mention(ent_id2mention_ls,
                                                      extract_sent_entity(pos_candi, _cur_aug_rep_pairs))
            me_target_id = random.choice([pos_candi["h"], pos_candi["t"]])
            ...
            noised_sentences, noised_s_ids = add_noise_sentence(noise_sent_num, item, new_sentences, shuffled_s_ids,
                                                                rep_pairs=_cur_aug_rep_pairs, shuffle=True)
            ...
            for _neg_sent in neg_ctx_sent:
                ...
                #                                            shuffle=True))
                negative_context.append(add_noise_sentence(noise_sent_num, item, _new_context_sentences, sent_order2s_id,
                                                           rep_pairs=_cur_aug_rep_pairs, shuffle=True)[0])
...
def read_examples(file_path: str, shuffle_context: bool = False,
    ...
    all_neg_candidates = {}
    all_neg_candidates_para = collections.defaultdict(dict)
    negative_pool = {}
    all_path_sentences = {}
    _neg_cnt_1 = 0
    _neg_cnt_2 = 0
    ...
    for x in tqdm(data, desc="preparing negative samples and candidates", total=len(data)):
        ...
        para_id = get_para_id(x)
        para_tmp = tmp[:] + all_path_sentences[x["id"]][:]
        all_neg_candidates_para[para_id][x["id"]] = para_tmp
    ...
    with Pool(num_workers, initializer=_initializer,
              initargs=(entity_pool, negative_pool, all_neg_candidates, all_path_sentences, all_neg_candidates_para,
        ...
        _annotate = partial(_process_single_item,
                            ...
                            remove_deduct=remove_deduct, remove_context=remove_context, para_sample_neg=para_sample_neg, )
        _results = list(tqdm(
...
def obtain_entity_spans(spans: List[str], entity_replacement: List[Dict], h_ent_id: str, t_ent_id: str, sent_id: int,
                        assertion: bool = False, noise_flag: bool = False):
    h_span_ids = set()
    t_span_ids = set()
    if not noise_flag:
        assert h_ent_id != t_ent_id, (h_ent_id, t_ent_id)
        ...
        if assertion:
            assert len(h_span_ids) >= 1, (spans, entity_replacement, h_ent_id, t_ent_id)
            assert len(t_span_ids) >= 1, (spans, entity_replacement, h_ent_id, t_ent_id)
    ...
    if assertion and (not noise_flag):
...
def obtain_sent_tokens(spans: List[str], entity_replacement: List[Dict], sent_id: int, mask: bool = False, me_target_id: str = None):
    if mask and me_target_id is not None:
        target_span_indices = []
        for re in entity_replacement:
            if re["id"] == me_target_id:
                target_span_indices.append(re["span_index"])
        assert len(target_span_indices)
        target_span_index = random.choice(target_span_indices)
    else:
        target_span_index = -1
    ...
    for span_id, span in enumerate(spans):
        ...
        if mask and me_target_id is not None and span_id == target_span_index:
            span_tokens = [_tokenizer.mask_token]
        else:
            span_tokens = _tokenizer.tokenize(span)
...
def prepare_context_h_t_spans(context_samples, option_sample, assertion: bool = False):
    ...
    for sent_id, sent in enumerate(context_samples):
        if "noise_flag" in sent:
            noise_flag = sent["noise_flag"]
        else:
            noise_flag = False
        sent_tokens, sent_h_spans, sent_t_spans = obtain_entity_spans(sent["spans"], sent["entity_replacement"],
                                                                      sent["h"], sent["t"], sent_id=sent_id,
                                                                      assertion=assertion, noise_flag=noise_flag)
...
def prepare_me_context(context_samples, option_sample, me_target_sent: Union[str, int], me_target_id: str):
    tokens = [_tokenizer.cls_token]

    for sent_id, sent in enumerate(context_samples):
        if me_target_sent == "pos" or me_target_sent != sent_id:
            mask = False
        else:
            mask = True
        sent_tokens = obtain_sent_tokens(sent["spans"], sent["entity_replacement"], sent_id, mask=mask, me_target_id=me_target_id)
        tokens.extend(sent_tokens)

    tokens.extend(get_sep_tokens(_tokenizer))

    if me_target_sent == "pos":
        mask = True
    else:
        mask = False
    op_tokens = obtain_sent_tokens(option_sample["spans"], option_sample["entity_replacement"], sent_id=0,
                                   mask=mask, me_target_id=me_target_id)
...
def length_filter_w_sentence_h_t_spans(sample, max_seq_length: int):
    ...
    if "negative_context" in sample:
        ...
        me_tokens = prepare_me_context(sample["context"], sample["condition"], sample["me_target_sent"], sample["me_target_id"])
        ...
        me_tokens = prepare_me_context(sample["context"], sample["positive"], sample["me_target_sent"], sample["me_target_id"])

    me_candidate_ent_str = {}
    for ent_id, ent_mentions in sample["ent_id2mention"].items():
        me_candidate_ent_str[ent_id] = random.choice(list(ent_mentions))
    ...
    sample["me_input"] = _tokenizer.convert_tokens_to_string(me_tokens)
    sample["me_candidate_ent"] = me_candidate_ent_str

    return True, sample


def get_para_id(example):
    exp_id = example["orig_id"] if "orig_id" in example else example["id"]
...
def convert_examples_into_features(file_path: str, tokenizer: PreTrainedTokenizer, id2rel_path_decode_id_file: str, rel_vocab: str,
    ...
    file_suffix = f"{tokenizer_name}_{shuffle_context}_{max_neg_num}_{aug_num}_" \
                  ...
                  f"{deduct_ratio}_{context_ratio}_{noise_sent_ratio}_{max_neg_samples_num}_{para_sample_neg}_" \
                  f"{'' if not remove_context else 'no-ctx-ex_'}{'' if not remove_deduct else 'no-duc-ex_'}path_v9.1.2_noise_me"
    ...
    examples, context_examples, raw_texts = read_examples(file_path, shuffle_context=shuffle_context, max_neg_num=max_neg_num,
                                                          ...
                                                          para_sample_neg=para_sample_neg,
