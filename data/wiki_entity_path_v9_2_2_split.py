...
logger = get_child_logger("Wiki.Entity.Path.V9.2.2.split")
...
def bias_entity_generate_rep_pairs(sentence, existing_ent_ids: set, exclude: bool = True):
    src_ent_ids = []
    if exclude:
...
def convert_examples_into_features(file_path: str, tokenizer: PreTrainedTokenizer, pattern_pair_file: str,
    ...
    file_suffix = f"{tokenizer_name}_{shuffle_context}_{max_neg_num}_{aug_num}_" \
                  ...
                  f"{'' if not remove_context else 'no-ctx-ex_'}{'' if not remove_deduct else 'no-duc-ex_'}path_v9.2.2_split"
