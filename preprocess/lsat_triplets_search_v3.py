...
def contains_entity(sentence: str, entity: str):
    entity = entity.strip()
    if len(entity) == 1:
        s = sentence.find(entity)
        while s != -1:
            a = not is_alphabet(sentence[s - 1]) if s > 0 else True
            b = not is_alphabet(sentence[s + 1]) if s + 1 < len(sentence) else True
            if a and b:
                return True
...
def find_triplets_ctx(item: Dict):
    ...
    for sent_id, sent in enumerate(sentences):
        if sent_id == 0:
            continue
        if sent_id <= 1 and (sum([1 for p in participants if contains_entity(sent, p)]) > (len(participants) // 2)
                             or "conditions" in sent or "constraints" in sent or "following" in sent):
            continue
        for part in participants:
            if contains_entity(sent, part):
                part2sent[part].append(sent_id)
        for pos in positions:
            if contains_entity(sent, pos):
...
def main():
    ...
    if args.output_file:
        ...
        output_file = args.input_file.replace(".json", f"_triplets_v3_type_{args.action}.json")
